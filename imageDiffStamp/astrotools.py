...
class AstroTools(object):
    ...
    def runHotpants(self, objImg, tmpImg, srcDir):
        ...
        convFile = "%s_%s_conv.fit"%(objpre,tmppre)
        convPath = "%s/%s"%(srcDir, convFile)
        ...
        #         '-tu','50000','-iu','50000','-nss','6','-ko','4','-bgo','1']
        cmd = [self.imgDiffProgram, '-inim', objFPath, '-tmplim', tmpFPath, '-outim', 
                 outFPath, '-oci', convPath, '-v', '0', '-nrx', '2', '-nry', '2', 
                 '-nsx', '4', '-nsy', '4', '-r', '6',
                 '-tu','10000','-iu','10000','-c','i']
        self.log.debug(cmd)
        ...
        return outFile, convFile, runSuccess
