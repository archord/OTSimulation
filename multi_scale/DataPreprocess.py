...
def getData3(totPath, rotPath, destPath, tNamePart, imgSize=64, transMethod='none'):
    ...
    if os.path.exists(timgbinPath):
        ...
        print("fotSize %d"%(fotImgs.shape[0]))
        timgbinPath = '%s/fotImgs_temp.npz'%(destPath)
        np.savez_compressed(timgbinPath, fotImgs=fotImgs, fs2ns=fs2ns)
        
        badImgs2 = np.array([])
        bads2ns2 = np.array([])
        for i, fname in enumerate(badImgfiles2):
            try:
                tpath21 = "%s/%s"%(fotPath, fname)
                print("%d:%s"%(i,tpath21))
                fdata1 = np.load(tpath21)
                ...
                if badImgs2.shape[0]==0:
                    badImgs2 = fotImg
                    bads2ns2 = fs2n
                else:
                    badImgs2 = np.concatenate((badImgs2, fotImg), axis=0)
                    bads2ns2 = np.concatenate((bads2ns2, fs2n), axis=0)
                if badImgs2.shape[0]>fotImgs.shape[0]:
                    break
            except Exception as e:
                tstr = traceback.format_exc()
                print(tstr)
        
        print("badImgs2 %d"%(badImgs2.shape[0]))
        timgbinPath = '%s/badImgs2_temp.npz'%(destPath)
        np.savez_compressed(timgbinPath, badImgs2=badImgs2, bads2ns2=bads2ns2)
        
        
        bad1Path = "%s/20190116bad"%(rotPath)
        badImgs1 = np.array([])
        bads2ns1 = np.array([])
        
        totNum = 0
        tdirs = os.listdir(bad1Path)
        for i, fname in enumerate(tdirs):
            try:
                if i%3!=1:
                    continue
                tpath21 = "%s/%s"%(bad1Path, fname)
                print("%d:%s"%(i,tpath21))
                fdata1 = np.load(tpath21)
                fotImg = fdata1['imgs']
                fprops = fdata1['parms']
                ...
                if badImgs1.shape[0]==0:
                    badImgs1 = fotImg
                    bads2ns1 = fs2n
                else:
                    badImgs1 = np.concatenate((badImgs1, fotImg), axis=0)
                    bads2ns1 = np.concatenate((bads2ns1, fs2n), axis=0)
                if badImgs1.shape[0]>10000:
                    break
            except Exception as e:
                tstr = traceback.format_exc()
                print(tstr)
                
        print("badImgs1 %d"%(badImgs1.shape[0]))
        timgbinPath = '%s/badImgs1_temp.npz'%(destPath)
        np.savez_compressed(timgbinPath, badImgs1=badImgs1, bads2ns1=bads2ns1)
        
        tot2Path = "%s/20190116tot"%(rotPath)
        tot2Imgs1 = np.array([])
        tot2s2ns1 = np.array([])
        
        totNum = 0
        tdirs = os.listdir(tot2Path)
        for i, fname in enumerate(tdirs):
            try:
                tpath21 = "%s/%s"%(tot2Path, fname)
                print("%d:%s"%(i,tpath21))
                fdata1 = np.load(tpath21)
                fotImg = fdata1['imgs']
                fprops = fdata1['parms']
                ...
                if tot2Imgs1.shape[0]==0:
                    tot2Imgs1 = fotImg
                    tot2s2ns1 = fs2n
                else:
                    tot2Imgs1 = np.concatenate((tot2Imgs1, fotImg), axis=0)
                    tot2s2ns1 = np.concatenate((tot2s2ns1, fs2n), axis=0)
                    
            except Exception as e:
                tstr = traceback.format_exc()
                print(tstr)
                
        print("tot2Imgs1 %d"%(tot2Imgs1.shape[0]))
        timgbinPath = '%s/tot2Imgs1_temp.npz'%(destPath)
        np.savez_compressed(timgbinPath, tot2Imgs1=tot2Imgs1, tot2s2ns1=tot2s2ns1)
        
        fotImgs = np.concatenate((fotImgs, badImgs2, tot2Imgs1, badImgs1), axis=0)
        fs2ns = np.concatenate((fs2ns, bads2ns2, tot2s2ns1, bads2ns1), axis=0)
        print("fotImgs %d"%(fotImgs.shape[0]))
        print("fs2ns %d"%(fs2ns.shape[0]))
        ...
        timgbinPath = '%s/totImgs1_temp.npz'%(destPath)
        np.savez_compressed(timgbinPath, totImgs=totImgs, ts2ns=ts2ns)
