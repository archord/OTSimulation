...
4, none: donot execuate any operation
'''
def getData(totPath, realFotPath, destPath, imgSize=8, transMethod='none'):
    
    timgbinPath = '%s/SIM_IMG_ALL_RealFOT_bin_%s%d.npz'%(destPath, transMethod, imgSize)
    if os.path.exists(timgbinPath):
        print("bin file exist, read from %s"%(timgbinPath))
        timgbin = np.load(timgbinPath)
        X = timgbin['X']
        ...
        for i, fname in enumerate(tdirs):
            if fname[:3]=='fot':
                tpath21 = "%s/%s"%(realFotPath, fname)
                print(tpath21)
                fdata1 = np.load(tpath21)
                fotImg = fdata1['imgs']
                fprops = fdata1['props']
                
                fotImg = getImgStamp(fotImg, size=imgSize, padding = 0, transMethod='none')
