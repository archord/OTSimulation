...
def backupDir(spath, dpath, logpath, ssh, ftp, ip, msgSession, stopDateStr, dataClient):
    ...
    for tdir in tdirs:
        if tdir[0]==tempDirName[0] and tdir[4]==tempDirName[4] and tdir[8]==tempDirName[8] and len(tdir)==len(tempDirName):
            ...
            #if tdateNumber<=stopDateNumber and tdateNumber>= 190923:
            if tdateNumber>= 190923:
    ...
    for tdir in dataDirs:
        ...
        #if dateStr>='190115' and dateStr<='190125':
        if dateStr<'190923':
           continue
        #logfile0.write("%s\n"%(tdir))
        logfile0.write("%s\n"%(dateStr))
...
def backupAllMachine(msgSession):
    ...
    dataClient = GWACDataClient('10.0.82.111', '8077')
    #dataClient = GWACDataClient('10.36.1.77', '8080')
...
if __name__ == '__main__':
    ...
    #while True:
        try:
            ...
            startDateTime = tDateTime.replace(hour=1, minute=0, second=0)
            endDateTime = tDateTime.replace(hour=23, minute=0, second=0)
